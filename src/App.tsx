  ...
  return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-gray-900 text-white">
        ...
        <div className="fixed inset-0 opacity-20">
          <div
              className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-blue-500 via-transparent to-transparent animate-pulse"></div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="min-h-screen flex items-center justify-center px-6">
            <div className="text-center">
              <div className="relative mb-8 inline-block">
                <div
                    className="absolute inset-0 animate-spin-slow bg-gradient-to-r from-blue-500 to-purple-500 rounded-full blur-lg opacity-50"></div>
                <img
                    src="/profil_pic.png"
                    alt="Maxime VÉRY"
              ...
              <h2 className="text-2xl mb-6 text-blue-200">Manager de Projet Informatique en Alternance</h2>
              <div className="flex items-center justify-center gap-2 mb-6">
                <MapPin size={20} className="text-blue-400"/>
                <span>Riom, Auvergne-Rhône-Alpes, France</span>
              </div>
              <div className="flex justify-center gap-6">
                <a href={socialLinks.github} target="_blank" rel="noopener noreferrer"
                   className="transform hover:scale-110 transition-transform">
                  <div className="bg-white/10 p-3 rounded-full hover:bg-white/20">
                    <Github size={24}/>
                  </div>
                </a>
          ...
          <div className="py-24 bg-black/30 backdrop-blur-sm">
            <div className="container mx-auto px-6">
              <blockquote className="text-2xl italic text-center max-w-4xl mx-auto">
                ...
                <span className="text-blue-400">"</span>
                <footer className="text-sm mt-4 text-blue-200">- Steve Jobs</footer>
              </blockquote>
            </div>
          </div>
              ...
              {activeTab === 'about' && (
                  <div className="space-y-8 animate-fadeIn">
                    ...
                    <div className="bg-white/5 p-6 rounded-lg mt-8">
                      <h3 className="text-xl font-bold text-blue-400 mb-4">À propos de moi</h3>
              ...
              {activeTab === 'experience' && (
                  <div className="space-y-8 pt-4 animate-fadeIn">
                    {experiences.map((exp, index) => (
                        <div key={index} className="border-l-4 border-blue-500 pl-6 relative mt-2">
                          ...
                          <div className="text-blue-200 mb-2">{exp.company} • {exp.startDate} - {exp.endDate}</div>
                          <div className="bg-white/5 p-4 rounded-lg mt-2">
                            {exp.description && <p className="text-gray-300 mb-3">{exp.description}</p>}
                            {exp.techStack && (
                                <div className="space-y-2">
                                  <div>
                                    <h4 className="text-blue-300 font-semibold">{exp.techStack.title}</h4>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="space-y-3 flex-1">
                              <div>
                                ...
                                {cert.code && <p className="text-blue-200">Code RNCP: {cert.code}</p>}
                                {cert.score && <p className="text-blue-200">{cert.score}</p>}
                                <p className="text-gray-300">{cert.issuer}</p>
                                {cert.level && <p className="text-gray-300">{cert.level}</p>}
                              </div>
                              {cert.details && (
                                  <div className="mt-4">
                                    <h4 className="text-sm font-semibold text-blue-300 mb-2">Compétences attestées:</h4>
                                    <ul className="list-disc list-inside text-gray-300 space-y-1">
                              ...
                              <div className="flex gap-2 text-sm text-gray-400">
                                ...
                                {cert.expiry && <span>• Expire : {cert.expiry}</span>}
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {contactInfo.map((info, index) => (
                          info.link ? (
                              <a
                                  ...
                                  className="block bg-white/5 p-6 rounded-lg hover:bg-white/10 transition-all duration-300 group"
                              >
                                <div className="flex items-start gap-4">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-gray-300 group-hover:text-blue-200 transition-colors block mb-2">
                  {info.value}
                </span>
                                    <p className="text-sm text-gray-400 group-hover:text-gray-300">{info.description}</p>
                                  </div>
                                </div>
                              </a>
                          ) : (
                              <div key={index} className="bg-white/5 p-6 rounded-lg hover:bg-white/10 transition-all duration-300">
                                <div className="flex items-start gap-4">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-gray-300 block mb-2">{info.value}</span>
                      ...
                      <div className="md:col-span-2 bg-white/5 p-6 rounded-lg">
                        <h3 className="text-xl font-bold text-blue-400 mb-4">Disponibilité</h3>
          ...
          <footer className="bg-black/30 backdrop-blur-sm py-8">
            <div className="container mx-auto px-6 text-center">
              ...
              <p className="text-blue-200">© 2024 Maxime VÉRY. All rights reserved.</p>
            </div>
          </footer>
        </div>
      </div>
